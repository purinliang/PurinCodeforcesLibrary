...
远古遗物模板虽然有 struct 或者 namespace 的包裹做到了复制粘贴后即可使用，但是充斥着大量的 C 语言的写法。首先是静态数组长度需要手动修改，对于某一些数据结构比如线段树或者后缀自动机，其包含的节点个数是数据规模的若干倍，每次都需要手动修改很容易改错而且很麻烦，对于可持久化数据结构节点的规模甚至容易算错。其次是对于类似 !u 和 !top 等 C 语言风格的代码的使用导致代码不容易理解。部分代码还因为没有验证到多组数据清空的问题导致异常（指普通平衡树），我意识到改为使用 vector 就可以方便清空。故打算对远古遗物模板进行重新开发和验证，称为第一代模板。

第一代模板的代码设计应遵循如下规则：

- 数组下标：所有的数组，无论是 C 语言的数组，还是 C++ 语言的 vector，都必须是以 1 作为第一个真实元素的下标。包括各类自动机等存在特殊节点的数据结构。使用从 1 开始的下标有利于我的惯性思维的理解。
- 区间：所有的区间都是闭区间，包括二分、三分、线段树，或者其他。使用闭区间有利于我的惯性思维的理解。
...
- 全局变量：不要在模板中使用全局变量，在 struct 和 class 中要规定好哪些是私有成员哪些是公有成员。使用 namespace 的写法极不方便，建议后续统一修改为 struct 的静态成员。
...
- 数组下标：所有的数组，无论是 C 语言的数组，还是 C++ 语言的 vector，都必须是以 0 作为第一个真实元素的下标。包括各类自动机等存在特殊节点的数据结构。使用从 0 开始的下标有利于简化代码长度以及和 C++ STL 形成统一风格。
- 区间：所有的区间都是前闭后开区间，包括二分、三分、线段树，或者其他。使用闭区间有利于有利于简化代码长度以及和 C++ STL 形成统一风格。
...
- 标准输入输出模板：应为所有的常用 STL 如各种类型的 vector，和 string 提供快速输入输出的方法。默认使用 cin / cout 作为输入方式，但是应该能一键重定向到 C 语言的输入输出方式。初步定义为 pin 和 pout。 
- 最小功能：对于 ST表 或者倍增 LCA 等算法，将不同的元素定义到一起会方便维护，但是每次却要注释掉不需要使用的部分，线段树尤其是其中的重灾区。暂时不清楚这样的功能是否是必要的。
- 效率：不包含输入输出的情况，应达到第一代模板 90% 的效率。 

第二代模板的开发工作不急着进行，更换模板需要一整套模板都进行更换（或者至少更换掉大部分常用的部分），否则会出现兼容问题。
