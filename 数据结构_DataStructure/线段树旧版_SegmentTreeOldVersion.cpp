   ...
   private:
    ...
    struct Node {
        ...
        string to_string() {
            ...
            string res = "";
            res += format("[{},{}]", std::to_string(_lidx), std::to_string(_ridx));
            res += format(", sum = {}", std::to_string(_sum));
#ifdef USING_MIN_MAX
            res += format(", min = {}", std::to_string(_min));
            res += format(", max = {}", std::to_string(_max));
#endif
            return res;
        }
